...
- Manual test deploy (optional): `rsync -az --delete site/ user@host:/var/www/life-hacks/`

## Content Style & Naming
- Write in clear, concise Markdown. Prefer sentence‑case headings.
- One `# H1` per page; subsequent sections use `##`/`###`.
- Use relative links and paths under the `/life-hacks/` prefix.
...
- Nginx must serve under `/life-hacks/` with `alias` (see `deploy/nginx/`).
